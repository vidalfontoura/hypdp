 ...
 * @author mvh
 */
public class TSHyperHeuristicAA extends HyperHeuristic {
	AcceptanceCriteria acceptanceCriteria;
	long starttimeincrease, starttimedecrease;
	long analysistime = 100;//millis

	public TSHyperHeuristicAA(long seed) {
		super(seed);
	}//end constructor
	...
	public void solve(ProblemDomain problem) {
		...
		starttimeincrease = System.currentTimeMillis();
		starttimedecrease = System.currentTimeMillis();
		acceptanceCriteria = new AcceptanceCriteria(rng);
		...
		LowLevelHeuristic[] heuristics = new LowLevelHeuristic[numberOfHeuristics-numberofxovers];//create the llh objects, but do not include the xovers
		int count = 0;
...
//			System.out.println("applied " + bestHeuristic.number);
			...
			if (delta > 0) {//if there is an increase in the objective function
				starttimeincrease = System.currentTimeMillis();//reset this because we only want to increasepercent when there is a period of continuous non improvement
			...
			if (System.currentTimeMillis() - starttimeincrease > analysistime) {
				acceptanceCriteria.increasePercent();
			}
			if (System.currentTimeMillis() - starttimedecrease > analysistime) {
				acceptanceCriteria.decreasePercent();
			}
	...
	public String toString() {
		return "Tabu Search Adaptive Acceptance";
	}
